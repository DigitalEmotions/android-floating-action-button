...
public class FloatingActionsMenu extends ViewGroup {
	...
	HashMap<View, ObjectAnimator> mExpandAlphaAnimators = new HashMap<>();
	HashMap<View, ObjectAnimator> mCollapseAlphaAnimators = new HashMap<>();
			...
			case EXPAND_DOWN:
				...
				for (int i = mButtonsCount - 1; i >= 0; i--) {
					...
					if (child instanceof FloatingActionButton) {
						...
						button.setAlpha(mExpanded ? button.getCustomAlpha() : 0f);
					} else {
					...
					params.setAnimationsTarget(child, true);
					...
					if (label != null) {
						...
						labelParams.setAnimationsTarget(label, true);
					}
	...
	protected class LayoutParams extends ViewGroup.LayoutParams {
		...
		Animator.AnimatorListener alphaListener = new Animator.AnimatorListener() {
			@Override
		...
		public void setAnimationsTarget(View view, boolean addAlpha) {

			if (view instanceof FloatingActionButton) {
				float alph = ((FloatingActionButton) view).getCustomAlpha();
				mCollapseAlpha.setProperty(View.ALPHA);
				mCollapseAlpha.setFloatValues(alph, 0f);
				mExpandAlpha.setProperty(View.ALPHA);
				mExpandAlpha.setFloatValues(0f, alph);
				mExpandAlpha.addListener(alphaListener);
				mCollapseAlpha.addListener(alphaListener);
			}

			mExpandAlphaAnimators.put(view,mExpandAlpha);
			mCollapseAlphaAnimators.put(view,mCollapseAlpha);
	...
	public void setButtonAnimationAlpha(){
		for (int i = mButtonsCount - 1; i >= 0; i--) {
			final View child = getChildAt(i);
			View label = (View) child.getTag(R.id.fab_label);
			float alpha = ((FloatingActionButton)child).getCustomAlpha();
			if(mExpandAlphaAnimators.containsKey(child)){
				ObjectAnimator anim = mExpandAlphaAnimators.get(child);
				anim.setFloatValues(0f, alpha);
			}
			if(mExpandAlphaAnimators.containsKey(label)){
				ObjectAnimator anim = mExpandAlphaAnimators.get(label);
				anim.setFloatValues(0f, alpha);
			}
			if(mCollapseAlphaAnimators.containsKey(child)){
				ObjectAnimator anim = mCollapseAlphaAnimators.get(child);
				anim.setFloatValues(alpha, 0f);
			}
			if(mCollapseAlphaAnimators.containsKey(label)){
				ObjectAnimator anim = mCollapseAlphaAnimators.get(label);
				anim.setFloatValues(alpha, 0f);
			}
	...
	public void cancelAnimation(){
		for(ObjectAnimator objectAnimator: mExpandAlphaAnimators.values()){
